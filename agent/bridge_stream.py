...
async def planner(state: State, writer: StreamWriter):
    ...
    conversation = "\n".join(
        f"{m.type}: {m.content}" for m in messages[:-1]  # 不包含最后一条
    )
    logger.info("历史对话:\n" + conversation)
...
def executor(state: State):
    ...
    # ---------- 空计划检查：如果计划为空，直接完成 ----------
    if not plan:
        logger.info("Executor: 计划为空，无需执行工具，直接进入总结阶段")
        return {
            "is_plan_completed": True,
            "next": "summarizer",
            "current_step": current_step,
            "plan": plan,  # 保留执行计划
            "messages": messages  # 保留对话历史
        }
...
async def router(state: State):
    ...
    if tool_fn and hasattr(tool_fn, "args_schema"):
        ...
        try:
            ...
            logger.info(f"Router: 获取本地工具 {tool_name} schema失败: {str(e)}")
    else:
        # 检查是否为MCP工具
        try:
            # 获取MCP工具列表（使用缓存）
    ...
    # 确保返回值包含plan和messages字段，以便执行计划和对话历史能够正确传递
    return {
        "current_step": current_step + 1,
        "result": new_result,
        "next": "executor",
...
async def checker(state: State):
    ...
    for m in reversed(messages):
        if isinstance(m, HumanMessage):
            ...
            break
    logger.info(f"Checker: 用户查询 - {user_query}")
...
async def summarizer(state: State, writer:StreamWriter):
    ...
    conversation = "\n".join(
        f"{m.type}: {m.content}" for m in final_messages[:-1]  # 不包含最后一条
    )
    # 使用从YAML加载的提示词
...
graph.add_conditional_edges(
    ...
    lambda x: x.get("next", "end"),
    {
        ...
        "summarizer": "summarizer",  # 空计划，跳到summarizer
        "end": END
    }
)
...
# 确保app_stream可以处理异步生成器节点
logger.info("BridgeStream: 图编译完成，支持流式输出")
...
#             # 更新final_state为最新的chunk
#             final_state = chunk
            
#             # 处理流式输出
#             if isinstance(chunk, dict):
#                 # 检查是否是custom_stream事件
...
#                         # 如果是部分内容，使用覆盖打印
#                         if is_partial:
#                             print(f"\r\033[K助手: {accumulated_reply}", end="", flush=True)
#                         else:
#                             # 完整内容，正常打印
#                             print(f"\n\n助手: {accumulated_reply}\n")
#     else:
#         # 不支持流式运行，使用普通调用
#         logger.info("=== 使用非流式运行")
